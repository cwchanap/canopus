...
use schema::{Message, Response, DaemonConfig, ClientConfig, Event, SystemState};
use std::fs;
use std::path::Path;
...
fn generate_typescript_definitions(schemas_dir: &Path) -> Result<()> {
    ...
    fs::create_dir_all(&ts_dir)
        .context("Failed to create TypeScript directory")?;
    ...
    typescript_content.push_str("  | { type: 'Status'; running: boolean; uptimeSeconds: number; version?: string };\n\n");
    ...
    typescript_content.push_str("  | { type: 'DaemonStarted'; timestamp: string; version: string }\n");
    typescript_content.push_str("  | { type: 'DaemonStopped'; timestamp: string }\n");
    typescript_content.push_str("  | { type: 'ClientConnected'; timestamp: string; clientId: string }\n");
    typescript_content.push_str("  | { type: 'ClientDisconnected'; timestamp: string; clientId: string }\n");
    typescript_content.push_str("  | { type: 'Custom'; eventType: string; timestamp: string; data: any };\n\n");
    ...
    fs::write(&ts_path, typescript_content)
        .context("Failed to write TypeScript definitions")?;
