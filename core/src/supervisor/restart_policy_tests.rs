...
use crate::supervisor::restart_policy::{RestartAction, RestartPolicyEngine};
...
mod tests {
    ...
    fn create_test_cases() -> Vec<RestartPolicyTestCase> {
        vec![
            ...
            RestartPolicyTestCase {
                ...
                exits: vec![TestExit::success(), TestExit::success()],
                expected_actions: vec![ExpectedAction::stop(), ExpectedAction::stop()],
            },
    ...
    fn test_restart_policy_table_driven() {
        ...
        for test_case in test_cases {
            ...
            let mut engine = RestartPolicyEngine::new(test_case.policy, test_case.backoff_config);

            assert_eq!(
                test_case.exits.len(),
                test_case.expected_actions.len(),
                "Test case '{}': exits and expected actions length mismatch",
                test_case.name
            );

            for (i, (exit, expected)) in test_case
                .exits
                .iter()
